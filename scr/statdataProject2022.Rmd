...
These plot show that there are a lot of (near) linear relationships between variables and that the discrete variables are important. It makes it easy to find interesting rows. ## intersting rows
...
The biplots aren't very clear and we can't recognize any categorical variables in them. Let's plot them again.
...
The only categorical variable you can recognize on the score plot was fuel the other if you would plot them are overlapping ruling them out. Meaning neither pam or kmeans recogized any categorical variables.

We would go with 1 cluster here.

## hierarchical clustering observations

TODO : do all types of clustering TODO : hdbscan ##\# hdbscan
...
## ANOVA
We don't know a fast way to do an ANOVA for lars in R. 
```{r}
...
```{r}
names(Xtrain.co2.lars.lasso)
# wut is the difference between SSE and RSS?
...
```
We can't get predict to work ... the way you could find a confidence interval is by using the fact that the conditional distribution of X is approximately normal (1 of the assumptions that we checked). The prediction gives the mean and the variance (which we assume to be constant) 
...
```{r}
Xtrain.knn <- vcr.knn.train(Xtrain[,c("engine_capacity","urban_metric","nox_emissions")], Xtrain$euro_standard, 25)
confmat.vcr(Xtrain.knn)
stackedplot(Xtrain.knn)
```
