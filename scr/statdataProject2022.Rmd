...
biplot(Xtrain.pca.scaleds,c(2,3))
```
...
Xtrain[bad_leverage_points,]
Xtrain[good_leverage_points,]
Xtrain[,0] 
names(Xtrain)
```
TODO discuss outliers
We are going to continue with the robust PCA.
...
```{r}
names(Xtrain.robpca)
Xtrain[1,][,continousCols]
Xtrain[1,][,continousCols]%*% Xtrain.robpca$loadings
```
