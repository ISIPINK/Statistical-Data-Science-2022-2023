...
print(summary(Xtrain.pca.unscaled))j
...
for (amount_centers in 1:6) {
  Xtrain.kmeans <-kmeans(Xtrain[,continousCols], amount_centers)
...
```{r}
for (amount_centers in 1:6) {
  Xtrain.pam <-pam(Xtrain[,continousCols], k = amount_centers, metric="manhattan")
...
### hdbscan
```{r}
for (pts in c(10,15,20,25,30)) {
  Xtrain.hdbscan <- hdbscan(Xtrain[,continousCols],minPts = pts)
  pairs(Xtrain.robpca.scores, col=Xtrain.hdbscan$cluster)
  plot(Xtrain.hdbscan)
}
```
### hclust
```{r}
for (mt in c("average","complete","single","median","ward.D")) {
  Xtrain.hclust <- hclust(dist(Xtrain[,continousCols]),method =mt)
